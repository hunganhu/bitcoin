...
public class MaxFeeTxHandler {
	 ...
	 * transactions, finds a set of valid transactions with maximum total 
	 * transaction fees, i.e. maximize the sum over all transactions 
	 * in the set of (sum of input values - sum of output values)),
	...
	public Transaction[] handleTxs(Transaction[] possibleTxs) {
		...
		HashMap<Transaction, Double> txMap = new HashMap<Transaction, Double>();
		
		for (Transaction tx : possibleTxs) {
			if (isValidTx(tx)) {
				txMap.put(tx, Double.valueOf(TxFee(tx)));
		...
		Double maxFee = (Collections.max(txMap.values())); // This will return max value in the Hashmap
		for (Map.Entry<Transaction, Double> entry : txMap.entrySet()) { // Iterate through hashmap
			if (entry.getValue() == maxFee) {
				validTx.add(entry.getKey());     // insert the transactions with max fee
			}
		}	
